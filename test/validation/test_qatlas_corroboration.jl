...
@testset "QAtlas corroboration (layer B)" begin
    ...
    @testset "Yang spontaneous magnetization vs MC (below Tc)" begin
        ...
        for T in Ts
            β = 1 / T
            M_yang = QAtlas.fetch(IsingSquare(), SpontaneousMagnetization(); β=β, J=1.0)
    ...
    @testset "critical exponent β consistency" begin
        ...
        alg = LocalUpdate(; rule=Metropolis(), selection=RandomSiteSelection())
        m_hi = mc_estimate(L, Thi, alg; R=6, burn=2500, nsteps=5000, seed0=8000).mAbsM
        m_lo = mc_estimate(L, Tlo, alg; R=6, burn=2500, nsteps=5000, seed0=8100).mAbsM
